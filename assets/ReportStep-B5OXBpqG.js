import{r as S,j as i,A as at,m as je}from"./motion-CZTIYVKD.js";import{P as ot,t as qe,w as Ae,K as nt,v as it,I as st,R as rt,J as lt,M as ct,N as mt,O as dt,Q as pt,r as Oe,E as ut,s as xt,G as ht,V as gt,C as Ve,a as We,b as Ue,B as Se,e as Ze}from"./index-MUuo-Ef1.js";import{F as He,X as $t,n as Pt,L as ft,l as Rt,T as Ft}from"./icons-ggQqh8om.js";import"./react-Bzgz95E1.js";import"./charts-wPg4HxFZ.js";function A(e,t){const a=e.find(F=>String(F.value)===String(t));return a?a.label:`Valor ${t}`}function _t(e){var k,v,T,_,E,y,D,N,O,x,j,Z,w,V,b,C,z,J,ne,Ne,le,ce,me,de,pe,ue,xe,he,ge,$e,Pe,fe,Re,Fe,_e,be,l,f,R;const{calculations:t,probability_calculations:a,loss_calculations:F,risk_results:c,frequency_results:m,probability_data:n,zones:u,selected_risk_components:L}=e,s=((k=u[0])==null?void 0:k.loss_data)||{};let r="";r+=`### Etapa — Probabilidade (P)
...
* *Fórmula:* PB = PB
* *Variáveis:*
    * PB (Nível do SPDA): **${n.PB}** (${A(qe,n.PB)})
...
* *Variáveis:*
    * PSPDₑ: **${n.PSPD_electric}** (${A(Ae,n.PSPD_electric)})
...
* *Variáveis:*
    * PSPDₐ: **${n.PSPD_data}** (${A(Ae,n.PSPD_data)})
...
* *Variáveis:*
    * PSPDₑ: **${n.PSPD_electric}** (${A(Ae,n.PSPD_electric)})
    * Ks1 (Malha wm1=${n.wm1}m): **${(y=a.Ks1)==null?void 0:y.toFixed(3)}**
    * Ks2 (Malha wm2=${n.wm2}m): **${(D=a.Ks2)==null?void 0:D.toFixed(3)}**
    * Ks3ₑ: **${n.Ks3_electric_int}** (${A(nt,n.Ks3_electric_int)})
    * Ks4ₑ (Uw=${n.Uw_electric_int}kV): **${(N=a.Ks4_electric)==null?void 0:N.toFixed(3)}**
* *Cálculo:* PM = ${n.PSPD_electric} × (${(O=a.Ks1)==null?void 0:O.toFixed(3)} × ${(x=a.Ks2)==null?void 0:x.toFixed(3)} × ${n.Ks3_electric_int} × ${(j=a.Ks4_electric)==null?void 0:j.toFixed(3)})²
...
* *Variáveis:*
    * PTU: **${n.PTU_electric}** (${A(it,n.PTU_electric)})
    * PEB: **${n.PEB_electric}** (${A(Ae,n.PEB_electric)})
    * PLD (ext): **${(w=n.PLD_electric_ext)==null?void 0:w.toFixed(2)}**
    * CLD (ext): **${n.CLD_electric_ext}**
* *Cálculo:* PU = ${n.PTU_electric} × ${n.PEB_electric} × ${(V=n.PLD_electric_ext)==null?void 0:V.toFixed(2)} × ${n.CLD_electric_ext}
...
* *Resultado:* **LA = ${(z=F.LA)==null?void 0:z.toExponential(3)}**
...
`;const M=Object.entries(L).filter(([,h])=>h).map(([h])=>{const o={RA:"RA = Nd × PA × LA",RB:"RB = Nd × PB × LB",RC:"RC = Nd × PC × LC",RM:"RM = Nm × PM × LM",RU:"RU = Nl × PU × LU",RV:"RV = Nl × PV × LV",RW:"RW = Nl × PW × LW",RZ:"RZ = Ni × PZ × LZ"},W=c[h];return`* **${h}:** ${o[h]||"N/A"} -> Resultado: **${W==null?void 0:W.toExponential(3)}**`}).join(`
`);r+=M+`
...
* *Cálculo:* FZ = (${(Fe=t.ni_electric)==null?void 0:Fe.toExponential(3)} × ${(_e=a.PZ)==null?void 0:_e.toExponential(3)}) + (${(be=t.ni_data)==null?void 0:be.toExponential(3)} × ${(l=a.PZT)==null?void 0:l.toExponential(3)})
...
* *Fórmula:* F = ${B.join(" + ")}
* *Cálculo:* F = ${B.map(h=>{var o;return((o=m[h])==null?void 0:o.toExponential(3))||0}).join(" + ")}
* *Resultado:* **F = ${(R=m.F)==null?void 0:R.toExponential(3)}**

`,r}async function bt(e){var le,ce,me,de,pe,ue,xe,he,ge,$e,Pe,fe,Re,Fe,_e,be;const{calculations:t,risk_results:a,frequency_results:F}=e,c={dados:1,parametros:2,calculos:3,resultados:4,parecer:5},m=_t(e),n=l=>{var te,Y;const f=Object.entries(l.risks_to_analyze).filter(([,p])=>p).map(([p])=>p),R={R1:1e-5,R3:.001,R4:.001},h=[];f.forEach(p=>{const I=l.risk_results[p]||0,q=R[p];I>q&&h.push(p)});const o=l.frequency_config.is_critical_system?.1:1,W=(((te=l.frequency_results)==null?void 0:te.F)||0)>o,U=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(p=>({k:p,v:l.risk_results[p]||0})).sort((p,I)=>I.v-p.v).slice(0,2).map(p=>{var I;return`${String(p.k)} (${(I=p.v)==null?void 0:I.toExponential(2)})`}).join(", "),H=`Este parecer foi elaborado com base na NBR 5419, considerando os riscos selecionados (${f.join(", ")||"nenhum"}) e a frequência de danos aos sistemas.`,Ce=`Frequência de Danos (F): **${(((Y=l.frequency_results)==null?void 0:Y.F)||0).toExponential(3)}**; Limite (FT): **${o.toFixed(1)}** → ${W?"**NÃO ACEITÁVEL**":"**ACEITÁVEL**"}.`,G=f.length?`Riscos totais: ${f.map(p=>`**${p}=${(l.risk_results[p]||0).toExponential(3)}** (RT=${R[p].toExponential(1)})`).join("; ")}.`:"Nenhum risco foi selecionado para análise próxima.",K=`Componentes dominantes observados: ${U}.`,se=`Recomendações prioritárias:
...
* Revisar parâmetros de ocupação e medidas internas (rf, hz, LO) e rotinas operacionais em zonas críticas.`,X=h.length||W?`Conclusão: Há itens **NÃO ACEITÁVEIS** (${[...h,W?"F":null].filter(Boolean).join(", ")}). Recomenda-se implementar as medidas acima e reprocessar a análise para verificar a redução dos componentes dominantes e a conformidade com os limites.`:"Conclusão: Todos os itens analisados estão **ACEITÁVEIS** frente aos limites. Recomenda-se manter as medidas propostas, documentação e manutenção periódica do sistema.";return[H,Ce,G,K,se,X].join(`

`)},u=Object.entries(e.risks_to_analyze).filter(([,l])=>l).map(([l],f)=>{var G,K,se,X,te,Y,p,I,q;const R=a[l]||0,h={R1:1e-5,R3:.001,R4:.001}[l]||0,o=R<=h,W={R1:"Perda de Vidas Humanas",R3:"Perda de Patrimônio Cultural",R4:"Perda de Valor Econômico"}[l],ie=o?"🟢 ✅":"🔴 ❌",P=e.selected_risk_components;let U="",H="";if(l==="R1"){const g=[],d=[];P.RA&&(g.push("RA"),d.push(`${(G=a.RA)==null?void 0:G.toExponential(3)}`)),P.RB&&(g.push("RB"),d.push(`${(K=a.RB)==null?void 0:K.toExponential(3)}`)),P.RC&&(g.push("RC"),d.push(`${(se=a.RC)==null?void 0:se.toExponential(3)}`)),P.RM&&(g.push("RM"),d.push(`${(X=a.RM)==null?void 0:X.toExponential(3)}`)),P.RU&&(g.push("RU"),d.push(`${((a.RU||0)+(a.RUT||0)).toExponential(3)}`)),P.RV&&(g.push("RV"),d.push(`${((a.RV||0)+(a.RVT||0)).toExponential(3)}`)),P.RW&&(g.push("RW"),d.push(`${((a.RW||0)+(a.RWT||0)).toExponential(3)}`)),P.RZ&&(g.push("RZ"),d.push(`${((a.RZ||0)+(a.RZT||0)).toExponential(3)}`)),U=g.join(" + "),H=`${d.join(" + ")} = ${R.toExponential(3)}`}else if(l==="R3"){const g=[],d=[];P.RB&&(g.push("RB3"),d.push(`${(te=a.RB3)==null?void 0:te.toExponential(3)}`)),P.RV&&(g.push("RV3"),d.push(`${((a.RV3||0)+(a.RVT3||0)).toExponential(3)}`)),U=g.join(" + "),H=`${d.join(" + ")} = ${R.toExponential(3)}`}else if(l==="R4"){const g=[],d=[];P.RA&&(g.push("RA4"),d.push(`${(Y=a.RA4)==null?void 0:Y.toExponential(3)}`)),P.RB&&(g.push("RB4"),d.push(`${(p=a.RB4)==null?void 0:p.toExponential(3)}`)),P.RC&&(g.push("RC4"),d.push(`${(I=a.RC4)==null?void 0:I.toExponential(3)}`)),P.RM&&(g.push("RM4"),d.push(`${(q=a.RM4)==null?void 0:q.toExponential(3)}`)),P.RU&&(g.push("RU4"),d.push(`${((a.RU4||0)+(a.RUT4||0)).toExponential(3)}`)),P.RV&&(g.push("RV4"),d.push(`${((a.RV4||0)+(a.RVT4||0)).toExponential(3)}`)),P.RW&&(g.push("RW4"),d.push(`${((a.RW4||0)+(a.RWT4||0)).toExponential(3)}`)),P.RZ&&(g.push("RZ4"),d.push(`${((a.RZ4||0)+(a.RZT4||0)).toExponential(3)}`)),U=g.join(" + "),H=`${d.join(" + ")} = ${R.toExponential(3)}`}const Ce=U?`
* *Composição (${l}):* ${U}
* *Cálculo:* ${H}`:"";return`### ${c.resultados}.${f+1}. Risco ${l} - ${W}
* **Risco Total Calculado (${l}):** **${R.toExponential(3)}**
* **Risco Tolerável (RT):** **${h.toExponential(1)}**
* **Resultado:** ${ie} O risco ${l} é **${o?"ACEITÁVEL":"NÃO ACEITÁVEL"}**.${Ce}`}).join(`

`),L=e.frequency_config.is_critical_system?.1:1,s=(F.F||0)<=L,r=s?"🟢 ✅":"🔴 ❌",M=l=>{try{return btoa(unescape(encodeURIComponent(l)))}catch{return""}},B=(l,f,R=720,h=300,o)=>{const ie=o!=null&&o.showLegend?72:48,P=44,U=8,H=f.length,Ce=R-80,G=h-ie-P,K=Math.max(10,Math.floor((Ce-(H-1)*U)/H)),se=typeof(o==null?void 0:o.toleranceLine)=="number"?Math.max(o.toleranceLine,0):0,X=$=>Math.log10(Math.max($,1e-12)),te=[...f.map($=>$.value),se].filter($=>$>0),Y=Math.max(...te.map($=>X($))),p=Math.min(...f.map($=>X($.value))),I=(o==null?void 0:o.barColor)||"#60A5FA",q=o==null?void 0:o.barColors,g=(o==null?void 0:o.bg)||"#FFFFFF",d="#111827",Ke="#475569",ze=typeof(o==null?void 0:o.labelAngle)=="number"?o.labelAngle:-35,Xe=`
  <defs>
  ...
  </defs>`;let Te="";f.forEach(($,re)=>{const ae=40+re*(K+U),Ee=(X($.value)-p)/Math.max(Y-p,1e-6),oe=Math.max(2,Math.floor(Ee*G)),Q=ie+(G-oe),Le=$.value.toExponential(2),ee=q?q[re%q.length]:I,ye=h-P+18,Be=ae+K/2,ve=ee==="#CBD5E1",Me=$.name==="R Total"||$.name==="F Total",Ye=ve?' stroke="#94a3b8" stroke-width="1" stroke-dasharray="2 2" fill-opacity="0.25"':Me?` stroke="${ee==="#10B981"?"#065f46":ee==="#EF4444"?"#7f1d1d":"#334155"}" stroke-width="3"`:"",Qe=Me?5:3,et=Me?12:10,tt=Me?"700":"400";Te+=`
  ...
  <text x="${ae+K/2}" y="${Q-6}" fill="${d}" font-size="${et}" font-weight="${tt}" text-anchor="middle">${Le}</text>`});let Ie="";if(typeof(o==null?void 0:o.toleranceLine)=="number"){const $=Math.max(o.toleranceLine,1e-12),re=(X($)-p)/Math.max(Y-p,1e-6),ae=Math.min(1,Math.max(0,re)),Ee=Math.max(2,Math.floor(ae*G)),oe=ie+(G-Ee);Ie=`
  <line x1="40" y1="${oe}" x2="${R-40}" y2="${oe}" stroke="#ef4444" stroke-width="2" stroke-dasharray="3 3" />`}let De="";if(!!(o!=null&&o.showLegend)&&q&&f.length){const $=typeof(o==null?void 0:o.legendBoxSize)=="number"?Math.max(6,o.legendBoxSize):10,re=typeof(o==null?void 0:o.legendColGap)=="number"?Math.max(2,o.legendColGap):8,ae=typeof(o==null?void 0:o.legendRowGap)=="number"?Math.max(2,o.legendRowGap):8,Ee=typeof(o==null?void 0:o.legendFontSize)=="number"?Math.max(8,o.legendFontSize):11,oe=Math.min(R-40,Math.max(120,(o==null?void 0:o.legendMaxWidth)||R-40));let Q=40,Le=46;f.forEach((ee,ye)=>{const Be=q[ye%q.length],ve=Math.min(160,Math.max(40,ee.name.length*7));Q+$+4+ve>oe&&(Q=40,Le+=$+ae),De+=`
...
<svg xmlns="http://www.w3.org/2000/svg" width="${R}" height="${h}" viewBox="0 0 ${R} ${h}">
  ${Xe}
  <rect x="0" y="0" width="${R}" height="${h}" fill="${g}"/>
  <text x="40" y="24" fill="${d}" font-size="14" font-weight="bold">${l}</text>
  ${De}
  <line x1="40" y1="${h-P}" x2="${R-40}" y2="${h-P}" stroke="${Ke}" stroke-width="1"/>
  ${Ie}
  ${Te}
</svg>`;return`data:image/svg+xml;base64,${M(Je)}`},v=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(l=>({name:l,value:e.risk_results[l]||1e-12,active:!!e.selected_risk_components[l]})),T=e.risks_to_analyze.R1?"R1":e.risks_to_analyze.R3?"R3":e.risks_to_analyze.R4?"R4":"R1",_=e.risk_results[T]||0,E=[...v.map(l=>({name:l.name,value:l.value})),{name:"R Total",value:_}],y=Object.entries(e.frequency_results||{}).filter(([l])=>l!=="F").map(([l,f])=>({name:l,value:f||0})),D=["#60A5FA","#A78BFA","#F472B6","#FB7185","#F59E0B","#34D399","#22D3EE","#93C5FD"],N="#CBD5E1",O=e.risks_to_analyze.R1?1e-5:e.risks_to_analyze.R3||e.risks_to_analyze.R4?.001:1e-5,j=_<=O?"#10B981":"#EF4444",Z=E.map((l,f)=>{var h;return l.name==="R Total"?j:((h=v[f])==null?void 0:h.active)?D[f%D.length]:N}),w=["#F87171","#60A5FA","#A78BFA","#34D399","#F59E0B","#FB7185","#64748B"],V=B("Componentes de Risco — Global",E,720,300,{barColors:Z,labelAngle:-30,legendMaxWidth:420,toleranceLine:O}),b=[...y,{name:"F Total",value:((le=e.frequency_results)==null?void 0:le.F)||0}],C=(((ce=e.frequency_results)==null?void 0:ce.F)||0)<=L,z="#CBD5E1",J=b.map((l,f)=>{if(l.name==="F Total")return C?"#10B981":"#EF4444";const R=w[f%w.length];return(l.value||0)>0?R:z}),ne=B("Frequência de Danos — Global",b,720,280,{barColors:J,labelAngle:-40,legendFontSize:10,legendMaxWidth:500,toleranceLine:L});return`
...
* **Frequência Calculada (F):** **${(be=F.F)==null?void 0:be.toExponential(3)}** danos/ano
...
`;continue}const L=r=>{let M=we(r);return M=M.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),M=M.replace(/(^|[^*])\*(?!\*)([^*]+?)\*(?!\*)/g,(B,k,v)=>`${k}<em>${v}</em>`),M};if(u.trim()==="---"){m&&(c+=`</ul>
`,m=!1),c+=`<hr style="border:0;border-top:1px solid #cbd5e1;margin:12px 0"/>
`;continue}const s=u.trim().match(/^!\[(.*?)\]\((.*?)\)$/);if(s){const[,r,M]=s;m&&(c+=`</ul>
`,m=!1);const B=n+1<F.length?F[n+1].trim():"",k=n-1>=0?F[n-1].trim():"",v=/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/;let T=null;const _=B.match(v),E=k.match(v);_?(T=`Figura ${_[2]} — ${_[3]}`.trim(),n+=1):E?T=`Figura ${E[2]} — ${E[3]}`.trim():r&&r.trim().length>0&&(T=r.trim());const y=T?`<figcaption>${L(T)}</figcaption>`:"";c+=`<figure style="margin:${t.figureMarginTopPx}px 0 ${t.figureMarginBottomPx}px"><img src="${we(M)}" alt="${we(r)}" style="max-width:100%;height:auto;border-radius:4px;border:none;display:inline-block;page-break-inside:avoid"/>${y}</figure>
...
`,m=!0);let r=u.trim().substring(2);for(;n+1<F.length&&F[n+1].startsWith("  ");)r+=" "+F[n+1].trim(),n++;const M=r.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),B=/\bresultado:\b/.test(M),k=/\bcalculo:\b/.test(M),v=/\bformula:\b/.test(M),T=/^\*\*[^*]+?\*\*:/.test(r.trim()),_=T?t.blockMarginBetweenItemsPx:Math.max(0,t.listItemMarginTopPx),E=T||B||k||v?t.blockMarginBetweenItemsPx:t.listItemMarginBottomPx,y=`${_}px 0 ${E}px`,D=/(F[óo]rmula:|C[áa]lculo:|Resultado:)/gi,N="§§",x=r.replace(D,N+"$1").split(N).filter(j=>j.trim().length>0);if(x.length>1||/:\s*$/.test(r.trim())){let j="";for(const Z of x){const w=Z.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),V=/\bresultado:\b/.test(w),b=/\bcalculo:\b/.test(w),C=/\bformula:\b/.test(w),z=V||b||C?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;j+=`<p style="margin:${z};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(Z)}</p>`,V&&(j+=`<p style="margin:0;height:${t.emptyLineHeightPx}px;line-height:0">&nbsp;</p>`)}c+=`<li style="margin:${y};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${j}</li>
`}else c+=`<li style="margin:${y};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(r)}</li>
`;continue}if(m&&(c+=`</ul>
`,m=!1),u.trim()){const r=n+1<F.length?F[n+1].trim():"";if(!(/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/.test(u.trim())&&/^!\[(.*?)\]\((.*?)\)$/.test(r))){const B=/(F[óo]rmula:|C[áa]lculo:|Resultado:)/gi,T=u.replace(B,"§§$1").split("§§").filter(_=>_.trim().length>0);if(T.length>1)for(const _ of T){const E=_.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),y=/\bresultado:\b/.test(E),D=/\bcalculo:\b/.test(E),N=/\bformula:\b/.test(E),O=y||D||N?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${O};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(_)}</p>
`,y&&(c+=`<p style="margin:0;height:${t.emptyLineHeightPx}px;line-height:0">&nbsp;</p>
`)}else{const _=u.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),E=/\bresultado:\b/.test(_),y=/\bcalculo:\b/.test(_),D=/\bformula:\b/.test(_),N=E||y||D?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${N};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(u)}</p>
`,E&&(c+=`<p style="margin:0;height:${t.emptyLineHeightPx}px;line-height:0">&nbsp;</p>
`)}}}}return m&&(c+=`</ul>
`),c};function At({data:e,onUpdate:t}){const[a,F]=S.useState(!1),[c,m]=S.useState(""),[n,u]=S.useState(!1),[L,s]=S.useState(22),[r,M]=S.useState(15),[B,k]=S.useState(22),[v,T]=S.useState(6),[_,E]=S.useState(22),[y,D]=S.useState(6),N="report_format_prefs",O=()=>{try{const b=localStorage.getItem(N);if(b){const C=JSON.parse(b);return{...Ge,...C}}}catch{}return Ge},[x]=S.useState(O()),j=async()=>{F(!0),m("");try{const b=await bt(e);m(b)}finally{F(!1)}},Z=async()=>{if(!c)return;const b=ke(c,x),C=c.replace(/\!\[[^\]]*\]\([^\)]*\)/g,"").replace(/^###\s+/gm,"").replace(/^##\s+/gm,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/^>\s+/gm,"").replace(/\n\n+/g,`

`);try{if(typeof window.ClipboardItem<"u"){const z=new window.ClipboardItem({"text/html":new Blob([b],{type:"text/html"}),"text/plain":new Blob([C],{type:"text/plain"})});await navigator.clipboard.write([z])}else await navigator.clipboard.writeText(C);u(!0),setTimeout(()=>u(!1),2e3)}catch{try{await navigator.clipboard.writeText(C)}catch{}u(!0),setTimeout(()=>u(!1),2e3)}},w=S.useMemo(()=>pt(e),[e.h,e.l,e.w,e.hp,e.ng,e.cd,e.has_electric_line,e.line_sections_1,e.use_adj_structure_1,e.l_adj_1,e.w_adj_1,e.h_adj_1,e.hp_adj_1,e.cd_adj_1,e.has_data_line,e.line_sections_2,e.use_adj_structure_2,e.l_adj_2,e.w_adj_2,e.h_adj_2,e.hp_adj_2,e.cd_adj_2]);S.useMemo(()=>Oe(e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),[e.probability_data,e.analyze_data_line_probabilities,e.has_data_line]);const V=S.useMemo(()=>e.zones.map(b=>{const C=ut(b),z=Oe(b.probability_data||e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),J=xt(z,b),ne=ht(w,J,C,e.selected_risk_components);return{zone:b,lossCalculations:C,riskCalculations:ne}}),[e.zones,w,e.selected_risk_components,e.analyze_data_line_probabilities,e.has_data_line,e.probability_data]);return S.useMemo(()=>gt(V),[V]),i.jsxs("div",{children:[i.jsxs(Ve,{children:[i.jsx(We,{className:"p-3",children:i.jsxs(Ue,{className:"flex items-center justify-between",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(He,{className:"w-5 h-5 text-slate-100"}),"Relatório Técnico Detalhado"]}),c&&!a&&i.jsx(Se,{variant:"outline",size:"icon",onClick:()=>m(""),className:"h-8 w-8 flex-shrink-0",children:i.jsx($t,{className:"w-4 h-4"})})]})}),c&&!a&&i.jsxs("div",{className:"flex justify-end gap-2 px-2 pb-0",children:[i.jsx(Se,{variant:"secondary",size:"sm",onClick:()=>{const b=ke(c,x),C=window.open("","_blank");if(!C)return;const z=Math.max(4,v),J=Math.max(4,y);C.document.write(`<!doctype html><html><head><meta charset="utf-8"/><title></title><style>
