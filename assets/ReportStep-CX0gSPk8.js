import{r as E,j as s,A as tt,m as ze}from"./motion-CZTIYVKD.js";import{P as at,t as Oe,w as Be,K as ot,v as nt,I as st,R as it,J as lt,M as rt,N as ct,O as dt,Q as mt,r as Ve,E as ut,s as pt,G as xt,V as ht,C as Ie,a as We,b as Ue,B as je,e as Ze}from"./index-48seSmin.js";import{F as Ge,X as $t,n as ft,L as gt,l as Rt,T as Pt}from"./icons-ggQqh8om.js";import"./react-Bzgz95E1.js";import"./charts-wPg4HxFZ.js";function F(e,d){const t=e.find(m=>String(m.value)===String(d));return t?t.label:`Valor ${d}`}function _t(e){var M,b,S,N,V,J,Y,Q,Z,ee,G,C,P,A,z,te,Ee,ve,Le,De,se,ie,le,re,ce,de,me,ue,pe,xe,he,$e,fe,ge,Re,Pe,i,g,R;const{calculations:d,probability_calculations:t,loss_calculations:m,risk_results:r,frequency_results:c,probability_data:n,zones:_,selected_risk_components:L}=e,a=((M=_[0])==null?void 0:M.loss_data)||{};let l="";l+=`### Etapa — Probabilidade (P)
...
* *Cálculo:* PU = ${n.PTU_electric} × ${n.PEB_electric} × ${(A=n.PLD_electric_ext)==null?void 0:A.toFixed(2)} × ${n.CLD_electric_ext}
...
`;const D=Object.entries(L).filter(([,x])=>x).map(([x])=>{const o={RA:"RA = Nd × PA × LA",RB:"RB = Nd × PB × LB",RC:"RC = Nd × PC × LC",RM:"RM = Nm × PM × LM",RU:"RU = Nl × PU × LU",RV:"RV = Nl × PV × LV",RW:"RW = Nl × PW × LW",RZ:"RZ = Ni × PZ × LZ"},B=r[x];return`* **${x}:** ${o[x]||"N/A"} -> Resultado: **${B==null?void 0:B.toExponential(3)}**`}).join(`
`);l+=D+`
...
`);const v=[];return e.frequency_config.has_equipment_in_ZPR0A&&v.push("FB"),v.push("FC","FM","FV","FW","FZ"),l+=`**F - Frequência Total de Danos**
* *Fórmula:* F = ${v.join(" + ")}
* *Cálculo:* F = ${v.map(x=>{var o;return((o=c[x])==null?void 0:o.toExponential(3))||0}).join(" + ")}
* *Resultado:* **F = ${(R=c.F)==null?void 0:R.toExponential(3)}**

`,l}async function Ct(e){var se,ie,le,re,ce,de,me,ue,pe,xe,he,$e,fe,ge,Re,Pe;const{calculations:d,risk_results:t,frequency_results:m}=e,r={dados:1,parametros:2,calculos:3,resultados:4,parecer:5},c=_t(e),n=i=>{var H,I;const g=Object.entries(i.risks_to_analyze).filter(([,p])=>p).map(([p])=>p),R={R1:1e-5,R3:.001,R4:.001},x=[];g.forEach(p=>{const T=i.risk_results[p]||0,y=R[p];T>y&&x.push(p)});const o=i.frequency_config.is_critical_system?.1:1,B=(((H=i.frequency_results)==null?void 0:H.F)||0)>o,j=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(p=>({k:p,v:i.risk_results[p]||0})).sort((p,T)=>T.v-p.v).slice(0,2).map(p=>{var T;return`${String(p.k)} (${(T=p.v)==null?void 0:T.toExponential(2)})`}).join(", "),w=`Este parecer foi elaborado com base na NBR 5419, considerando os riscos selecionados (${g.join(", ")||"nenhum"}) e a frequência de danos aos sistemas.`,_e=`Frequência de Danos (F): **${(((I=i.frequency_results)==null?void 0:I.F)||0).toExponential(3)}**; Limite (FT): **${o.toFixed(1)}** → ${B?"**NÃO ACEITÁVEL**":"**ACEITÁVEL**"}.`,k=g.length?`Riscos totais: ${g.map(p=>`**${p}=${(i.risk_results[p]||0).toExponential(3)}** (RT=${R[p].toExponential(1)})`).join("; ")}.`:"Nenhum risco foi selecionado para análise próxima.",q=`Componentes dominantes observados: ${j}.`,oe=`Recomendações prioritárias:
...
* Revisar parâmetros de ocupação e medidas internas (rf, hz, LO) e rotinas operacionais em zonas críticas.`,O=x.length||B?`Conclusão: Há itens **NÃO ACEITÁVEIS** (${[...x,B?"F":null].filter(Boolean).join(", ")}). Recomenda-se implementar as medidas acima e reprocessar a análise para verificar a redução dos componentes dominantes e a conformidade com os limites.`:"Conclusão: Todos os itens analisados estão **ACEITÁVEIS** frente aos limites. Recomenda-se manter as medidas propostas, documentação e manutenção periódica do sistema.";return[w,_e,k,q,oe,O].join(`

`)},_=Object.entries(e.risks_to_analyze).filter(([,i])=>i).map(([i],g)=>{var k,q,oe,O,H,I,p,T,y;const R=t[i]||0,x={R1:1e-5,R3:.001,R4:.001}[i]||0,o=R<=x,B={R1:"Perda de Vidas Humanas",R3:"Perda de Patrimônio Cultural",R4:"Perda de Valor Econômico"}[i],ae=o?"🟢 ✅":"🔴 ❌",f=e.selected_risk_components;let j="",w="";if(i==="R1"){const h=[],u=[];f.RA&&(h.push("RA"),u.push(`${(k=t.RA)==null?void 0:k.toExponential(3)}`)),f.RB&&(h.push("RB"),u.push(`${(q=t.RB)==null?void 0:q.toExponential(3)}`)),f.RC&&(h.push("RC"),u.push(`${(oe=t.RC)==null?void 0:oe.toExponential(3)}`)),f.RM&&(h.push("RM"),u.push(`${(O=t.RM)==null?void 0:O.toExponential(3)}`)),f.RU&&(h.push("RU"),u.push(`${((t.RU||0)+(t.RUT||0)).toExponential(3)}`)),f.RV&&(h.push("RV"),u.push(`${((t.RV||0)+(t.RVT||0)).toExponential(3)}`)),f.RW&&(h.push("RW"),u.push(`${((t.RW||0)+(t.RWT||0)).toExponential(3)}`)),f.RZ&&(h.push("RZ"),u.push(`${((t.RZ||0)+(t.RZT||0)).toExponential(3)}`)),j=h.join(" + "),w=`${u.join(" + ")} = ${R.toExponential(3)}`}else if(i==="R3"){const h=[],u=[];f.RB&&(h.push("RB3"),u.push(`${(H=t.RB3)==null?void 0:H.toExponential(3)}`)),f.RV&&(h.push("RV3"),u.push(`${((t.RV3||0)+(t.RVT3||0)).toExponential(3)}`)),j=h.join(" + "),w=`${u.join(" + ")} = ${R.toExponential(3)}`}else if(i==="R4"){const h=[],u=[];f.RA&&(h.push("RA4"),u.push(`${(I=t.RA4)==null?void 0:I.toExponential(3)}`)),f.RB&&(h.push("RB4"),u.push(`${(p=t.RB4)==null?void 0:p.toExponential(3)}`)),f.RC&&(h.push("RC4"),u.push(`${(T=t.RC4)==null?void 0:T.toExponential(3)}`)),f.RM&&(h.push("RM4"),u.push(`${(y=t.RM4)==null?void 0:y.toExponential(3)}`)),f.RU&&(h.push("RU4"),u.push(`${((t.RU4||0)+(t.RUT4||0)).toExponential(3)}`)),f.RV&&(h.push("RV4"),u.push(`${((t.RV4||0)+(t.RVT4||0)).toExponential(3)}`)),f.RW&&(h.push("RW4"),u.push(`${((t.RW4||0)+(t.RWT4||0)).toExponential(3)}`)),f.RZ&&(h.push("RZ4"),u.push(`${((t.RZ4||0)+(t.RZT4||0)).toExponential(3)}`)),j=h.join(" + "),w=`${u.join(" + ")} = ${R.toExponential(3)}`}const _e=j?`
* *Composição (${i}):* ${j}
* *Cálculo:* ${w}`:"";return`### ${r.resultados}.${g+1}. Risco ${i} - ${B}
...
`),L=e.frequency_config.is_critical_system?.1:1,a=(m.F||0)<=L,l=a?"🟢 ✅":"🔴 ❌",D=i=>{try{return btoa(unescape(encodeURIComponent(i)))}catch{return""}},v=(i,g,R=720,x=300,o)=>{const ae=o!=null&&o.showLegend?72:48,f=44,j=8,w=g.length,_e=R-80,k=x-ae-f,q=Math.max(10,Math.floor((_e-(w-1)*j)/w)),oe=typeof(o==null?void 0:o.toleranceLine)=="number"?Math.max(o.toleranceLine,0):0,O=$=>Math.log10(Math.max($,1e-12)),H=[...g.map($=>$.value),oe].filter($=>$>0),I=Math.max(...H.map($=>O($))),p=Math.min(...g.map($=>O($.value))),T=(o==null?void 0:o.barColor)||"#60A5FA",y=o==null?void 0:o.barColors,h=(o==null?void 0:o.bg)||"#FFFFFF",u="#111827",He="#475569",Me=typeof(o==null?void 0:o.labelAngle)=="number"?o.labelAngle:-35,Xe=`
  <defs>
  ...
  <line x1="40" y1="${K}" x2="${R-40}" y2="${K}" stroke="#ef4444" stroke-width="2" stroke-dasharray="3 3" />`}let Se="";if(!!(o!=null&&o.showLegend)&&y&&g.length){const $=typeof(o==null?void 0:o.legendBoxSize)=="number"?Math.max(6,o.legendBoxSize):10,ne=typeof(o==null?void 0:o.legendColGap)=="number"?Math.max(2,o.legendColGap):8,X=typeof(o==null?void 0:o.legendRowGap)=="number"?Math.max(2,o.legendRowGap):8,Ce=typeof(o==null?void 0:o.legendFontSize)=="number"?Math.max(8,o.legendFontSize):11,K=Math.min(R-40,Math.max(120,(o==null?void 0:o.legendMaxWidth)||R-40));let W=40,Fe=46;g.forEach((U,Te)=>{const ye=y[Te%y.length],Ne=Math.min(160,Math.max(40,U.name.length*7));W+$+4+Ne>K&&(W=40,Fe+=$+X),Se+=`
...
<svg xmlns="http://www.w3.org/2000/svg" width="${R}" height="${x}" viewBox="0 0 ${R} ${x}">
  ...
  ${qe}
  ${Ae}
</svg>`;return`data:image/svg+xml;base64,${D(Ke)}`},b=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(i=>({name:i,value:e.risk_results[i]||1e-12,active:!!e.selected_risk_components[i]})),S=e.risks_to_analyze.R1?"R1":e.risks_to_analyze.R3?"R3":e.risks_to_analyze.R4?"R4":"R1",N=e.risk_results[S]||0,V=[...b.map(i=>({name:i.name,value:i.value})),{name:"R Total",value:N}],J=Object.entries(e.frequency_results||{}).filter(([i])=>i!=="F").map(([i,g])=>({name:i,value:g||0})),Y=["#60A5FA","#A78BFA","#F472B6","#FB7185","#F59E0B","#34D399","#22D3EE","#93C5FD"],Q="#CBD5E1",Z=e.risks_to_analyze.R1?1e-5:e.risks_to_analyze.R3||e.risks_to_analyze.R4?.001:1e-5,G=N<=Z?"#10B981":"#EF4444",C=V.map((i,g)=>{var x;return i.name==="R Total"?G:((x=b[g])==null?void 0:x.active)?Y[g%Y.length]:Q}),P=["#F87171","#60A5FA","#A78BFA","#34D399","#F59E0B","#FB7185","#64748B"],A=v("Componentes de Risco — Global",V,720,300,{barColors:C,labelAngle:-30,legendMaxWidth:420,toleranceLine:Z}),z=[...J,{name:"F Total",value:((se=e.frequency_results)==null?void 0:se.F)||0}],te=(((ie=e.frequency_results)==null?void 0:ie.F)||0)<=L,Ee="#CBD5E1",ve=z.map((i,g)=>{if(i.name==="F Total")return te?"#10B981":"#EF4444";const R=P[g%P.length];return(i.value||0)>0?R:Ee}),Le=v("Frequência de Danos — Global",z,720,280,{barColors:ve,labelAngle:-40,legendFontSize:10,legendMaxWidth:500,toleranceLine:L});return`
...
`).split(`
`);let m="",r=!1;for(let c=0;c<t.length;c++){let n=t[c];const _=a=>{let l=we(a);return l=l.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),l=l.replace(/(^|[^*])\*(?!\*)([^*]+?)\*(?!\*)/g,(D,v,M)=>`${v}<em>${M}</em>`),l};if(n.trim()==="---"){r&&(m+=`</ul>
`,r=!1),m+=`<hr style="border:0;border-top:1px solid #cbd5e1;margin:12px 0"/>
`;continue}const L=n.trim().match(/^!\[(.*?)\]\((.*?)\)$/);if(L){const[,a,l]=L;r&&(m+=`</ul>
`,r=!1);const D=c+1<t.length?t[c+1].trim():"",v=c-1>=0?t[c-1].trim():"",M=/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/;let b=null;const S=D.match(M),N=v.match(M);S?(b=`Figura ${S[2]} — ${S[3]}`.trim(),c+=1):N?b=`Figura ${N[2]} — ${N[3]}`.trim():a&&a.trim().length>0&&(b=a.trim());const V=b?`<figcaption>${_(b)}</figcaption>`:"";m+=`<figure style="margin:12px 0"><img src="${we(l)}" alt="${we(a)}" style="max-width:100%;height:auto;border-radius:4px;border:none;display:inline-block;page-break-inside:avoid"/>${V}</figure>
...
`,r=!0);let a=n.trim().substring(2);for(;c+1<t.length&&t[c+1].startsWith("  ");)a+=" "+t[c+1].trim(),c++;const D=/\bresultado:\b/i.test(a.replace(/\*/g,""))?"6px 0 14px":"4px 0";m+=`<li style="margin:${D};font-size:11pt;line-height:1.35">${_(a)}</li>
`;continue}if(r&&(m+=`</ul>
`,r=!1),n.trim()){const a=c+1<t.length?t[c+1].trim():"";/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/.test(n.trim())&&/^!\[(.*?)\]\((.*?)\)$/.test(a)||(m+=`<p style="margin:8px 0;font-size:11pt;line-height:1.35">${_(n)}</p>
`)}}return r&&(m+=`</ul>
`),m};function Nt({data:e,onUpdate:d}){const[t,m]=E.useState(!1),[r,c]=E.useState(""),[n,_]=E.useState(!1),[L,a]=E.useState(22),[l,D]=E.useState(15),[v,M]=E.useState(22),[b,S]=E.useState(6),[N,V]=E.useState(22),[J,Y]=E.useState(6),Q=async()=>{m(!0),c("");try{const C=await Ct(e);c(C)}finally{m(!1)}},Z=async()=>{if(!r)return;const C=ke(r),P=r.replace(/\!\[[^\]]*\]\([^\)]*\)/g,"").replace(/^###\s+/gm,"").replace(/^##\s+/gm,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/^>\s+/gm,"").replace(/\n\n+/g,`

`);try{if(typeof window.ClipboardItem<"u"){const A=new window.ClipboardItem({"text/html":new Blob([C],{type:"text/html"}),"text/plain":new Blob([P],{type:"text/plain"})});await navigator.clipboard.write([A])}else await navigator.clipboard.writeText(P);_(!0),setTimeout(()=>_(!1),2e3)}catch{try{await navigator.clipboard.writeText(P)}catch{}_(!0),setTimeout(()=>_(!1),2e3)}},ee=E.useMemo(()=>mt(e),[e.h,e.l,e.w,e.hp,e.ng,e.cd,e.has_electric_line,e.line_sections_1,e.use_adj_structure_1,e.l_adj_1,e.w_adj_1,e.h_adj_1,e.hp_adj_1,e.cd_adj_1,e.has_data_line,e.line_sections_2,e.use_adj_structure_2,e.l_adj_2,e.w_adj_2,e.h_adj_2,e.hp_adj_2,e.cd_adj_2]);E.useMemo(()=>Ve(e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),[e.probability_data,e.analyze_data_line_probabilities,e.has_data_line]);const G=E.useMemo(()=>e.zones.map(C=>{const P=ut(C),A=Ve(C.probability_data||e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),z=pt(A,C),te=xt(ee,z,P,e.selected_risk_components);return{zone:C,lossCalculations:P,riskCalculations:te}}),[e.zones,ee,e.selected_risk_components,e.analyze_data_line_probabilities,e.has_data_line,e.probability_data]);return E.useMemo(()=>ht(G),[G]),s.jsxs("div",{children:[s.jsxs(Ie,{children:[s.jsx(We,{className:"p-3",children:s.jsxs(Ue,{className:"flex items-center justify-between",children:[s.jsxs("div",{className:"flex items-center gap-2",children:[s.jsx(Ge,{className:"w-5 h-5 text-slate-100"}),"Relatório Técnico Detalhado"]}),r&&!t&&s.jsx(je,{variant:"outline",size:"icon",onClick:()=>c(""),className:"h-8 w-8 flex-shrink-0",children:s.jsx($t,{className:"w-4 h-4"})})]})}),r&&!t&&s.jsxs("div",{className:"flex justify-end gap-2 px-2 pb-0",children:[s.jsx(je,{variant:"secondary",size:"sm",onClick:()=>{const C=ke(r),P=window.open("","_blank");if(!P)return;const A=Math.max(4,b),z=Math.max(4,J);P.document.write(`<!doctype html><html><head><meta charset="utf-8"/><title></title><style>
@page{margin:${L}mm ${l}mm;}
...
main{padding:${A}mm ${l}mm ${z}mm; overflow:visible;}
