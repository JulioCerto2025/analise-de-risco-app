...
`),L=e.frequency_config.is_critical_system?.1:1,s=(P.F||0)<=L,r=s?"🟢 ✅":"🔴 ❌",M=l=>{try{return btoa(unescape(encodeURIComponent(l)))}catch{return""}},B=(l,R,F=720,h=300,n)=>{const ie=n!=null&&n.showLegend?72:48,f=44,U=8,Z=R.length,Ce=F-80,G=h-ie-f,K=Math.max(10,Math.floor((Ce-(Z-1)*U)/Z)),se=typeof(n==null?void 0:n.toleranceLine)=="number"?Math.max(n.toleranceLine,0):0,X=$=>Math.log10(Math.max($,1e-12)),te=[...R.map($=>$.value),se].filter($=>$>0),Y=Math.max(...te.map($=>X($))),p=Math.min(...R.map($=>X($.value))),q=(n==null?void 0:n.barColor)||"#60A5FA",O=n==null?void 0:n.barColors,g=(n==null?void 0:n.bg)||"#FFFFFF",d="#111827",Ke="#475569",ze=typeof(n==null?void 0:n.labelAngle)=="number"?n.labelAngle:-35,Xe=`
  ...
  <text x="${ae+K/2}" y="${Q-6}" fill="${d}" font-size="${et}" font-weight="${tt}" text-anchor="middle">${Le}</text>`});let Ie="";if(typeof(n==null?void 0:n.toleranceLine)=="number"){const $=Math.max(n.toleranceLine,1e-12),re=(X($)-p)/Math.max(Y-p,1e-6),ae=Math.min(1,Math.max(0,re)),Ee=Math.max(2,Math.floor(ae*G)),oe=ie+(G-Ee);Ie=`
...
`.trim()}const we=e=>e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),Ge={paragraphFontSizePt:11,paragraphLineHeight:1.35,paragraphMarginTopPx:6,paragraphMarginBottomPx:8,listItemMarginTopPx:4,listItemMarginBottomPx:4,listItemExtraMarginBottomPx:10,blockMarginBetweenItemsPx:16,listMarginTopPx:8,listMarginBottomPx:12,listPaddingLeftPx:18,h2FontSizeRem:1.25,h2MarginTopPx:14,h2MarginBottomPx:10,h3FontSizeRem:1.1,h3MarginTopPx:10,h3MarginBottomPx:8,figureMarginTopPx:10,figureMarginBottomPx:10,emptyLineHeightPx:16},ke=(e,t)=>{if(!e)return"";const P=e.replace(/\\n/g,`
`).split(`
`);let c="",m=!1;for(let o=0;o<P.length;o++){let u=P[o];if(u.trim()===""){m?c+=`<li style="list-style:none;margin:${t.listItemMarginTopPx}px 0 ${t.listItemMarginBottomPx}px"><span style="display:inline-block;height:${t.emptyLineHeightPx}px"></span></li>
...
`;continue}const L=r=>{let M=we(r);return M=M.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),M=M.replace(/(^|[^*])\*(?!\*)([^*]+?)\*(?!\*)/g,(B,I,v)=>`${I}<em>${v}</em>`),M};if(u.trim()==="---"){m&&(c+=`</ul>
