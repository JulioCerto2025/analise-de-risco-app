import{r as A,j as i,A as at,m as De}from"./motion-CZTIYVKD.js";import{P as ot,t as qe,w as Be,K as nt,v as it,I as st,R as rt,J as lt,M as ct,N as dt,O as mt,Q as pt,r as Oe,E as ut,s as xt,G as ht,V as $t,C as Ve,a as We,b as Ue,B as Ne,e as Ze}from"./index-DkFFuS8s.js";import{F as He,X as gt,n as ft,L as Pt,l as Rt,T as Ft}from"./icons-ggQqh8om.js";import"./react-Bzgz95E1.js";import"./charts-wPg4HxFZ.js";function v(e,t){const a=e.find(R=>String(R.value)===String(t));return a?a.label:`Valor ${t}`}function _t(e){var y,L,M,B,j,V,ee,te,K,F,ae,oe,W,X,_,C,N,U,ne,Se,le,ce,de,me,pe,ue,xe,he,$e,ge,fe,Pe,Re,Fe,_e,Ce,l,f,P;const{calculations:t,probability_calculations:a,loss_calculations:R,risk_results:c,frequency_results:d,probability_data:n,zones:u,selected_risk_components:T}=e,s=((y=u[0])==null?void 0:y.loss_data)||{};let r="";r+=`### Etapa — Probabilidade (P)
...
* *Variáveis:*
    * PSPDₑ: **${n.PSPD_electric}** (${v(Be,n.PSPD_electric)})
...
* *Variáveis:*
    * PSPDₐ: **${n.PSPD_data}** (${v(Be,n.PSPD_data)})
...
* *Variáveis:*
    ...
    * PEB: **${n.PEB_electric}** (${v(Be,n.PEB_electric)})
...
* *Resultado:* **LA = ${(N=R.LA)==null?void 0:N.toExponential(3)}**
...
`)},u=Object.entries(e.risks_to_analyze).filter(([,l])=>l).map(([l],f)=>{var I,q,se,O,J,Z,p,S,z;const P=a[l]||0,x={R1:1e-5,R3:.001,R4:.001}[l]||0,o=P<=x,D={R1:"Perda de Vidas Humanas",R3:"Perda de Patrimônio Cultural",R4:"Perda de Valor Econômico"}[l],ie=o?"🟢 ✅":"🔴 ❌",g=e.selected_risk_components;let w="",k="";if(l==="R1"){const h=[],m=[];g.RA&&(h.push("RA"),m.push(`${(I=a.RA)==null?void 0:I.toExponential(3)}`)),g.RB&&(h.push("RB"),m.push(`${(q=a.RB)==null?void 0:q.toExponential(3)}`)),g.RC&&(h.push("RC"),m.push(`${(se=a.RC)==null?void 0:se.toExponential(3)}`)),g.RM&&(h.push("RM"),m.push(`${(O=a.RM)==null?void 0:O.toExponential(3)}`)),g.RU&&(h.push("RU"),m.push(`${((a.RU||0)+(a.RUT||0)).toExponential(3)}`)),g.RV&&(h.push("RV"),m.push(`${((a.RV||0)+(a.RVT||0)).toExponential(3)}`)),g.RW&&(h.push("RW"),m.push(`${((a.RW||0)+(a.RWT||0)).toExponential(3)}`)),g.RZ&&(h.push("RZ"),m.push(`${((a.RZ||0)+(a.RZT||0)).toExponential(3)}`)),w=h.join(" + "),k=`${m.join(" + ")} = ${P.toExponential(3)}`}else if(l==="R3"){const h=[],m=[];g.RB&&(h.push("RB3"),m.push(`${(J=a.RB3)==null?void 0:J.toExponential(3)}`)),g.RV&&(h.push("RV3"),m.push(`${((a.RV3||0)+(a.RVT3||0)).toExponential(3)}`)),w=h.join(" + "),k=`${m.join(" + ")} = ${P.toExponential(3)}`}else if(l==="R4"){const h=[],m=[];g.RA&&(h.push("RA4"),m.push(`${(Z=a.RA4)==null?void 0:Z.toExponential(3)}`)),g.RB&&(h.push("RB4"),m.push(`${(p=a.RB4)==null?void 0:p.toExponential(3)}`)),g.RC&&(h.push("RC4"),m.push(`${(S=a.RC4)==null?void 0:S.toExponential(3)}`)),g.RM&&(h.push("RM4"),m.push(`${(z=a.RM4)==null?void 0:z.toExponential(3)}`)),g.RU&&(h.push("RU4"),m.push(`${((a.RU4||0)+(a.RUT4||0)).toExponential(3)}`)),g.RV&&(h.push("RV4"),m.push(`${((a.RV4||0)+(a.RVT4||0)).toExponential(3)}`)),g.RW&&(h.push("RW4"),m.push(`${((a.RW4||0)+(a.RWT4||0)).toExponential(3)}`)),g.RZ&&(h.push("RZ4"),m.push(`${((a.RZ4||0)+(a.RZT4||0)).toExponential(3)}`)),w=h.join(" + "),k=`${m.join(" + ")} = ${P.toExponential(3)}`}const be=w?`
* *Composição (${l}):* ${w}
...
`),T=e.frequency_config.is_critical_system?.1:1,s=(R.F||0)<=T,r=s?"🟢 ✅":"🔴 ❌",b=l=>{try{return btoa(unescape(encodeURIComponent(l)))}catch{return""}},E=(l,f,P=720,x=300,o)=>{const ie=o!=null&&o.showLegend?72:48,g=44,w=8,k=f.length,be=P-80,I=x-ie-g,q=Math.max(10,Math.floor((be-(k-1)*w)/k)),se=typeof(o==null?void 0:o.toleranceLine)=="number"?Math.max(o.toleranceLine,0):0,O=$=>Math.log10(Math.max($,1e-12)),J=[...f.map($=>$.value),se].filter($=>$>0),Z=Math.max(...J.map($=>O($))),p=Math.min(...f.map($=>O($.value))),S=(o==null?void 0:o.barColor)||"#60A5FA",z=o==null?void 0:o.barColors,h=(o==null?void 0:o.bg)||"#FFFFFF",m="#111827",Ke="#475569",ze=typeof(o==null?void 0:o.labelAngle)=="number"?o.labelAngle:-35,Xe=`
  <defs>
  ...
  <line x1="40" y1="${Q}" x2="${P-40}" y2="${Q}" stroke="#ef4444" stroke-width="2" stroke-dasharray="3 3" />`}let je="";if(!!(o!=null&&o.showLegend)&&z&&f.length){const $=typeof(o==null?void 0:o.legendBoxSize)=="number"?Math.max(6,o.legendBoxSize):10,re=typeof(o==null?void 0:o.legendColGap)=="number"?Math.max(2,o.legendColGap):8,Y=typeof(o==null?void 0:o.legendRowGap)=="number"?Math.max(2,o.legendRowGap):8,Ee=typeof(o==null?void 0:o.legendFontSize)=="number"?Math.max(8,o.legendFontSize):11,Q=Math.min(P-40,Math.max(120,(o==null?void 0:o.legendMaxWidth)||P-40));let H=40,Le=46;f.forEach((G,ve)=>{const Ae=z[ve%z.length],ye=Math.min(160,Math.max(40,G.name.length*7));H+$+4+ye>Q&&(H=40,Le+=$+Y),je+=`
...
<svg xmlns="http://www.w3.org/2000/svg" width="${P}" height="${x}" viewBox="0 0 ${P} ${x}">
  ...
  ${Ie}
  ${Me}
</svg>`;return`data:image/svg+xml;base64,${b(Je)}`},L=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(l=>({name:l,value:e.risk_results[l]||1e-12,active:!!e.selected_risk_components[l]})),M=e.risks_to_analyze.R1?"R1":e.risks_to_analyze.R3?"R3":e.risks_to_analyze.R4?"R4":"R1",B=e.risk_results[M]||0,j=[...L.map(l=>({name:l.name,value:l.value})),{name:"R Total",value:B}],V=Object.entries(e.frequency_results||{}).filter(([l])=>l!=="F").map(([l,f])=>({name:l,value:f||0})),ee=["#60A5FA","#A78BFA","#F472B6","#FB7185","#F59E0B","#34D399","#22D3EE","#93C5FD"],te="#CBD5E1",K=e.risks_to_analyze.R1?1e-5:e.risks_to_analyze.R3||e.risks_to_analyze.R4?.001:1e-5,ae=B<=K?"#10B981":"#EF4444",oe=j.map((l,f)=>{var x;return l.name==="R Total"?ae:((x=L[f])==null?void 0:x.active)?ee[f%ee.length]:te}),W=["#F87171","#60A5FA","#A78BFA","#34D399","#F59E0B","#FB7185","#64748B"],X=E("Componentes de Risco — Global",j,720,300,{barColors:oe,labelAngle:-30,legendMaxWidth:420,toleranceLine:K}),_=[...V,{name:"F Total",value:((le=e.frequency_results)==null?void 0:le.F)||0}],C=(((ce=e.frequency_results)==null?void 0:ce.F)||0)<=T,N="#CBD5E1",U=_.map((l,f)=>{if(l.name==="F Total")return C?"#10B981":"#EF4444";const P=W[f%W.length];return(l.value||0)>0?P:N}),ne=E("Frequência de Danos — Global",_,720,280,{barColors:U,labelAngle:-40,legendFontSize:10,legendMaxWidth:500,toleranceLine:T});return`
...
`,d=!1);const E=n+1<R.length?R[n+1].trim():"",y=n-1>=0?R[n-1].trim():"",L=/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/;let M=null;const B=E.match(L),j=y.match(L);B?(M=`Figura ${B[2]} — ${B[3]}`.trim(),n+=1):j?M=`Figura ${j[2]} — ${j[3]}`.trim():r&&r.trim().length>0&&(M=r.trim());const V=M?`<figcaption>${T(M)}</figcaption>`:"";c+=`<figure style="margin:${t.figureMarginTopPx}px 0 ${t.figureMarginBottomPx}px"><img src="${we(b)}" alt="${we(r)}" style="max-width:100%;height:auto;border-radius:4px;border:none;display:inline-block;page-break-inside:avoid"/>${V}</figure>
...
`,d=!0);let r=u.trim().substring(2);for(;n+1<R.length&&R[n+1].startsWith("  ");)r+=" "+R[n+1].trim(),n++;const b=r.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),E=/\bresultado:\b/.test(b),y=/\bcalculo:\b/.test(b),L=/\bformula:\b/.test(b),M=E||y||L?`${Math.max(0,t.listItemMarginTopPx)}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.listItemMarginTopPx}px 0 ${t.listItemMarginBottomPx}px`;c+=`<li style="margin:${M};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${T(r)}</li>
`;continue}if(d&&(c+=`</ul>
`,d=!1),u.trim()){const r=n+1<R.length?R[n+1].trim():"";if(!(/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/.test(u.trim())&&/^!\[(.*?)\]\((.*?)\)$/.test(r))){const E=u.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),y=/\bresultado:\b/.test(E),L=/\bcalculo:\b/.test(E),M=/\bformula:\b/.test(E),B=y||L||M?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${B};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${T(u)}</p>
`}}}return d&&(c+=`</ul>
`),c};function Bt({data:e,onUpdate:t}){const[a,R]=A.useState(!1),[c,d]=A.useState(""),[n,u]=A.useState(!1),[T,s]=A.useState(22),[r,b]=A.useState(15),[E,y]=A.useState(22),[L,M]=A.useState(6),[B,j]=A.useState(22),[V,ee]=A.useState(6),te="report_format_prefs",K=()=>{try{const _=localStorage.getItem(te);if(_){const C=JSON.parse(_);return{...Ge,...C}}}catch{}return Ge},[F]=A.useState(K()),ae=async()=>{R(!0),d("");try{const _=await Ct(e);d(_)}finally{R(!1)}},oe=async()=>{if(!c)return;const _=ke(c,F),C=c.replace(/\!\[[^\]]*\]\([^\)]*\)/g,"").replace(/^###\s+/gm,"").replace(/^##\s+/gm,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/^>\s+/gm,"").replace(/\n\n+/g,`

`);try{if(typeof window.ClipboardItem<"u"){const N=new window.ClipboardItem({"text/html":new Blob([_],{type:"text/html"}),"text/plain":new Blob([C],{type:"text/plain"})});await navigator.clipboard.write([N])}else await navigator.clipboard.writeText(C);u(!0),setTimeout(()=>u(!1),2e3)}catch{try{await navigator.clipboard.writeText(C)}catch{}u(!0),setTimeout(()=>u(!1),2e3)}},W=A.useMemo(()=>pt(e),[e.h,e.l,e.w,e.hp,e.ng,e.cd,e.has_electric_line,e.line_sections_1,e.use_adj_structure_1,e.l_adj_1,e.w_adj_1,e.h_adj_1,e.hp_adj_1,e.cd_adj_1,e.has_data_line,e.line_sections_2,e.use_adj_structure_2,e.l_adj_2,e.w_adj_2,e.h_adj_2,e.hp_adj_2,e.cd_adj_2]);A.useMemo(()=>Oe(e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),[e.probability_data,e.analyze_data_line_probabilities,e.has_data_line]);const X=A.useMemo(()=>e.zones.map(_=>{const C=ut(_),N=Oe(_.probability_data||e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),U=xt(N,_),ne=ht(W,U,C,e.selected_risk_components);return{zone:_,lossCalculations:C,riskCalculations:ne}}),[e.zones,W,e.selected_risk_components,e.analyze_data_line_probabilities,e.has_data_line,e.probability_data]);return A.useMemo(()=>$t(X),[X]),i.jsxs("div",{children:[i.jsxs(Ve,{children:[i.jsx(We,{className:"p-3",children:i.jsxs(Ue,{className:"flex items-center justify-between",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(He,{className:"w-5 h-5 text-slate-100"}),"Relatório Técnico Detalhado"]}),c&&!a&&i.jsx(Ne,{variant:"outline",size:"icon",onClick:()=>d(""),className:"h-8 w-8 flex-shrink-0",children:i.jsx(gt,{className:"w-4 h-4"})})]})}),c&&!a&&i.jsxs("div",{className:"flex justify-end gap-2 px-2 pb-0",children:[i.jsx(Ne,{variant:"secondary",size:"sm",onClick:()=>{const _=ke(c,F),C=window.open("","_blank");if(!C)return;const N=Math.max(4,L),U=Math.max(4,V);C.document.write(`<!doctype html><html><head><meta charset="utf-8"/><title></title><style>
...
main{padding:${N}mm ${r}mm ${U}mm; overflow:visible;}
