...
* *Variáveis:*
    * PSPDₑ: **${n.PSPD_electric}** (${B(Be,n.PSPD_electric)})
    * Ks1 (Malha wm1=${n.wm1}m): **${(z=a.Ks1)==null?void 0:z.toFixed(3)}**
    * Ks2 (Malha wm2=${n.wm2}m): **${(N=a.Ks2)==null?void 0:N.toFixed(3)}**
    * Ks3ₑ: **${n.Ks3_electric_int}** (${B(nt,n.Ks3_electric_int)})
    * Ks4ₑ (Uw=${n.Uw_electric_int}kV): **${(y=a.Ks4_electric)==null?void 0:y.toFixed(3)}**
...
* *Fórmula:* FB = Nd × PB
* *Cálculo:* FB = ${(ze=t.nd)==null?void 0:ze.toExponential(3)} × ${a.PB}
...
* *Fórmula:* F = ${v.join(" + ")}
* *Cálculo:* F = ${v.map(g=>{var o;return((o=m[g])==null?void 0:o.toExponential(3))||0}).join(" + ")}
...
`),L=e.frequency_config.is_critical_system?.1:1,s=(F.F||0)<=L,r=s?"🟢 ✅":"🔴 ❌",T=l=>{try{return btoa(unescape(encodeURIComponent(l)))}catch{return""}},v=(l,f,R=720,g=300,o)=>{const ie=o!=null&&o.showLegend?72:48,P=44,V=8,U=f.length,be=R-80,Z=g-ie-P,H=Math.max(10,Math.floor((be-(U-1)*V)/U)),se=typeof(o==null?void 0:o.toleranceLine)=="number"?Math.max(o.toleranceLine,0):0,G=$=>Math.log10(Math.max($,1e-12)),te=[...f.map($=>$.value),se].filter($=>$>0),Y=Math.max(...te.map($=>G($))),p=Math.min(...f.map($=>G($.value))),k=(o==null?void 0:o.barColor)||"#60A5FA",I=o==null?void 0:o.barColors,h=(o==null?void 0:o.bg)||"#FFFFFF",d="#111827",Ke="#475569",Ne=typeof(o==null?void 0:o.labelAngle)=="number"?o.labelAngle:-35,Xe=`
  <defs>
  ...
  <text x="${ve}" y="${ye}" fill="${d}" font-size="11" text-anchor="${Ne<0?"end":Ne>0?"start":"middle"}" transform="rotate(${Ne} ${ve} ${ye})">${$.name}</text>`,Me+=`
...
<svg xmlns="http://www.w3.org/2000/svg" width="${R}" height="${g}" viewBox="0 0 ${R} ${g}">
  ...
  ${Ie}
  ${Me}
</svg>`;return`data:image/svg+xml;base64,${T(Je)}`},A=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(l=>({name:l,value:e.risk_results[l]||1e-12,active:!!e.selected_risk_components[l]})),M=e.risks_to_analyze.R1?"R1":e.risks_to_analyze.R3?"R3":e.risks_to_analyze.R4?"R4":"R1",_=e.risk_results[M]||0,C=[...A.map(l=>({name:l.name,value:l.value})),{name:"R Total",value:_}],z=Object.entries(e.frequency_results||{}).filter(([l])=>l!=="F").map(([l,f])=>({name:l,value:f||0})),N=["#60A5FA","#A78BFA","#F472B6","#FB7185","#F59E0B","#34D399","#22D3EE","#93C5FD"],y="#CBD5E1",D=e.risks_to_analyze.R1?1e-5:e.risks_to_analyze.R3||e.risks_to_analyze.R4?.001:1e-5,K=_<=D?"#10B981":"#EF4444",X=C.map((l,f)=>{var g;return l.name==="R Total"?K:((g=A[f])==null?void 0:g.active)?N[f%N.length]:y}),q=["#F87171","#60A5FA","#A78BFA","#34D399","#F59E0B","#FB7185","#64748B"],W=v("Componentes de Risco — Global",C,720,300,{barColors:X,labelAngle:-30,legendMaxWidth:420,toleranceLine:D}),b=[...z,{name:"F Total",value:((le=e.frequency_results)==null?void 0:le.F)||0}],E=(((ce=e.frequency_results)==null?void 0:ce.F)||0)<=L,w="#CBD5E1",J=b.map((l,f)=>{if(l.name==="F Total")return E?"#10B981":"#EF4444";const R=q[f%q.length];return(l.value||0)>0?R:w}),ne=v("Frequência de Danos — Global",b,720,280,{barColors:J,labelAngle:-40,legendFontSize:10,legendMaxWidth:500,toleranceLine:L});return`
...
`;continue}const L=r=>{let T=we(r);return T=T.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),T=T.replace(/(^|[^*])\*(?!\*)([^*]+?)\*(?!\*)/g,(v,j,A)=>`${j}<em>${A}</em>`),T};if(u.trim()==="---"){m&&(c+=`</ul>
`,m=!1),c+=`<hr style="border:0;border-top:1px solid #cbd5e1;margin:12px 0"/>
`;continue}const s=u.trim().match(/^!\[(.*?)\]\((.*?)\)$/);if(s){const[,r,T]=s;m&&(c+=`</ul>
`,m=!1);const v=n+1<F.length?F[n+1].trim():"",j=n-1>=0?F[n-1].trim():"",A=/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/;let M=null;const _=v.match(A),C=j.match(A);_?(M=`Figura ${_[2]} — ${_[3]}`.trim(),n+=1):C?M=`Figura ${C[2]} — ${C[3]}`.trim():r&&r.trim().length>0&&(M=r.trim());const z=M?`<figcaption>${L(M)}</figcaption>`:"";c+=`<figure style="margin:${t.figureMarginTopPx}px 0 ${t.figureMarginBottomPx}px"><img src="${we(T)}" alt="${we(r)}" style="max-width:100%;height:auto;border-radius:4px;border:none;display:inline-block;page-break-inside:avoid"/>${z}</figure>
...
`,m=!0);let r=u.trim().substring(2);for(;n+1<F.length&&F[n+1].startsWith("  ");)r+=" "+F[n+1].trim(),n++;const T=r.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),v=/\bresultado:\b/.test(T),j=/\bcalculo:\b/.test(T),A=/\bformula:\b/.test(T),M=v||j||A?`${Math.max(0,t.listItemMarginTopPx)}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.listItemMarginTopPx}px 0 ${t.listItemMarginBottomPx}px`,_=/(F[óo]rmula:|C[áa]lculo:|Resultado:)/gi,C="§§",N=r.replace(_,C+"$1").split(C).filter(y=>y.trim().length>0);if(N.length>1||/:\s*$/.test(r.trim())){let y="";for(const D of N){const x=D.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),K=/\bresultado:\b/.test(x),X=/\bcalculo:\b/.test(x),q=/\bformula:\b/.test(x),W=K||X||q?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;y+=`<p style="margin:${W};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(D)}</p>`,y+=`<p style="margin:${t.emptyLineHeightPx}px 0 ${t.emptyLineHeightPx}px;font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">&nbsp;</p>`}c+=`<li style="margin:${M};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${y}</li>
`}else c+=`<li style="margin:${M};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(r)}</li>
`;continue}if(m&&(c+=`</ul>
`,m=!1),u.trim()){const r=n+1<F.length?F[n+1].trim():"";if(!(/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/.test(u.trim())&&/^!\[(.*?)\]\((.*?)\)$/.test(r))){const v=/(F[óo]rmula:|C[áa]lculo:|Resultado:)/gi,M=u.replace(v,"§§$1").split("§§").filter(_=>_.trim().length>0);if(M.length>1)for(const _ of M){const C=_.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),z=/\bresultado:\b/.test(C),N=/\bcalculo:\b/.test(C),y=/\bformula:\b/.test(C),D=z||N||y?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${D};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(_)}</p>
`}else{const _=u.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),C=/\bresultado:\b/.test(_),z=/\bcalculo:\b/.test(_),N=/\bformula:\b/.test(_),y=C||z||N?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${y};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(u)}</p>
`}}}}return m&&(c+=`</ul>
`),c};function Bt({data:e,onUpdate:t}){const[a,F]=S.useState(!1),[c,m]=S.useState(""),[n,u]=S.useState(!1),[L,s]=S.useState(22),[r,T]=S.useState(15),[v,j]=S.useState(22),[A,M]=S.useState(6),[_,C]=S.useState(22),[z,N]=S.useState(6),y="report_format_prefs",D=()=>{try{const b=localStorage.getItem(y);if(b){const E=JSON.parse(b);return{...Ge,...E}}}catch{}return Ge},[x]=S.useState(D()),K=async()=>{F(!0),m("");try{const b=await Ct(e);m(b)}finally{F(!1)}},X=async()=>{if(!c)return;const b=ke(c,x),E=c.replace(/\!\[[^\]]*\]\([^\)]*\)/g,"").replace(/^###\s+/gm,"").replace(/^##\s+/gm,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/^>\s+/gm,"").replace(/\n\n+/g,`

`);try{if(typeof window.ClipboardItem<"u"){const w=new window.ClipboardItem({"text/html":new Blob([b],{type:"text/html"}),"text/plain":new Blob([E],{type:"text/plain"})});await navigator.clipboard.write([w])}else await navigator.clipboard.writeText(E);u(!0),setTimeout(()=>u(!1),2e3)}catch{try{await navigator.clipboard.writeText(E)}catch{}u(!0),setTimeout(()=>u(!1),2e3)}},q=S.useMemo(()=>pt(e),[e.h,e.l,e.w,e.hp,e.ng,e.cd,e.has_electric_line,e.line_sections_1,e.use_adj_structure_1,e.l_adj_1,e.w_adj_1,e.h_adj_1,e.hp_adj_1,e.cd_adj_1,e.has_data_line,e.line_sections_2,e.use_adj_structure_2,e.l_adj_2,e.w_adj_2,e.h_adj_2,e.hp_adj_2,e.cd_adj_2]);S.useMemo(()=>Oe(e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),[e.probability_data,e.analyze_data_line_probabilities,e.has_data_line]);const W=S.useMemo(()=>e.zones.map(b=>{const E=ut(b),w=Oe(b.probability_data||e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),J=xt(w,b),ne=gt(q,J,E,e.selected_risk_components);return{zone:b,lossCalculations:E,riskCalculations:ne}}),[e.zones,q,e.selected_risk_components,e.analyze_data_line_probabilities,e.has_data_line,e.probability_data]);return S.useMemo(()=>ht(W),[W]),i.jsxs("div",{children:[i.jsxs(Ve,{children:[i.jsx(We,{className:"p-3",children:i.jsxs(Ue,{className:"flex items-center justify-between",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(He,{className:"w-5 h-5 text-slate-100"}),"Relatório Técnico Detalhado"]}),c&&!a&&i.jsx(Se,{variant:"outline",size:"icon",onClick:()=>m(""),className:"h-8 w-8 flex-shrink-0",children:i.jsx($t,{className:"w-4 h-4"})})]})}),c&&!a&&i.jsxs("div",{className:"flex justify-end gap-2 px-2 pb-0",children:[i.jsx(Se,{variant:"secondary",size:"sm",onClick:()=>{const b=ke(c,x),E=window.open("","_blank");if(!E)return;const w=Math.max(4,A),J=Math.max(4,z);E.document.write(`<!doctype html><html><head><meta charset="utf-8"/><title></title><style>
