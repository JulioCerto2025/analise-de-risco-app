import{r as S,j as i,A as at,m as je}from"./motion-CZTIYVKD.js";import{P as ot,t as qe,w as Ae,K as nt,v as it,I as st,R as rt,J as lt,M as ct,N as mt,O as dt,Q as pt,r as Oe,E as ut,s as xt,G as gt,V as ht,C as Ve,a as We,b as Ue,B as Se,e as Ze}from"./index-BtEQ4kFD.js";import{F as He,X as $t,n as Pt,L as ft,l as Rt,T as Ft}from"./icons-ggQqh8om.js";import"./react-Bzgz95E1.js";import"./charts-wPg4HxFZ.js";function A(e,t){const a=e.find(F=>String(F.value)===String(t));return a?a.label:`Valor ${t}`}function _t(e){var w,y,T,_,E,v,D,N,O,x,k,U,j,Z,b,C,z,J,ne,Ne,le,ce,me,de,pe,ue,xe,ge,he,$e,Pe,fe,Re,Fe,_e,be,l,f,R;const{calculations:t,probability_calculations:a,loss_calculations:F,risk_results:c,frequency_results:m,probability_data:n,zones:u,selected_risk_components:L}=e,s=((w=u[0])==null?void 0:w.loss_data)||{};let r="";r+=`### Etapa — Probabilidade (P)
...
* *Resultado:* **PA = ${(y=a.PA)==null?void 0:y.toExponential(3)}**
...
* *Cálculo:* PM = ${n.PSPD_electric} × (${(O=a.Ks1)==null?void 0:O.toFixed(3)} × ${(x=a.Ks2)==null?void 0:x.toFixed(3)} × ${n.Ks3_electric_int} × ${(k=a.Ks4_electric)==null?void 0:k.toFixed(3)})²
* *Resultado:* **PM = ${(U=a.PM)==null?void 0:U.toExponential(3)}**
...
* *Variáveis:*
    ...
    * PLD (ext): **${(j=n.PLD_electric_ext)==null?void 0:j.toFixed(2)}**
...
`),L=e.frequency_config.is_critical_system?.1:1,s=(F.F||0)<=L,r=s?"🟢 ✅":"🔴 ❌",M=l=>{try{return btoa(unescape(encodeURIComponent(l)))}catch{return""}},B=(l,f,R=720,g=300,o)=>{const ie=o!=null&&o.showLegend?72:48,P=44,W=8,H=f.length,Ce=R-80,G=g-ie-P,K=Math.max(10,Math.floor((Ce-(H-1)*W)/H)),se=typeof(o==null?void 0:o.toleranceLine)=="number"?Math.max(o.toleranceLine,0):0,X=$=>Math.log10(Math.max($,1e-12)),te=[...f.map($=>$.value),se].filter($=>$>0),Y=Math.max(...te.map($=>X($))),p=Math.min(...f.map($=>X($.value))),I=(o==null?void 0:o.barColor)||"#60A5FA",q=o==null?void 0:o.barColors,h=(o==null?void 0:o.bg)||"#FFFFFF",d="#111827",Ke="#475569",ze=typeof(o==null?void 0:o.labelAngle)=="number"?o.labelAngle:-35,Xe=`
  <defs>
  ...
  </defs>`;let Te="";f.forEach(($,re)=>{const ae=40+re*(K+W),Ee=(X($.value)-p)/Math.max(Y-p,1e-6),oe=Math.max(2,Math.floor(Ee*G)),Q=ie+(G-oe),Le=$.value.toExponential(2),ee=q?q[re%q.length]:I,Be=g-P+18,ye=ae+K/2,ve=ee==="#CBD5E1",Me=$.name==="R Total"||$.name==="F Total",Ye=ve?' stroke="#94a3b8" stroke-width="1" stroke-dasharray="2 2" fill-opacity="0.25"':Me?` stroke="${ee==="#10B981"?"#065f46":ee==="#EF4444"?"#7f1d1d":"#334155"}" stroke-width="3"`:"",Qe=Me?5:3,et=Me?12:10,tt=Me?"700":"400";Te+=`
...
<svg xmlns="http://www.w3.org/2000/svg" width="${R}" height="${g}" viewBox="0 0 ${R} ${g}">
  ...
  ${Ie}
  ${Te}
</svg>`;return`data:image/svg+xml;base64,${M(Je)}`},y=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(l=>({name:l,value:e.risk_results[l]||1e-12,active:!!e.selected_risk_components[l]})),T=e.risks_to_analyze.R1?"R1":e.risks_to_analyze.R3?"R3":e.risks_to_analyze.R4?"R4":"R1",_=e.risk_results[T]||0,E=[...y.map(l=>({name:l.name,value:l.value})),{name:"R Total",value:_}],v=Object.entries(e.frequency_results||{}).filter(([l])=>l!=="F").map(([l,f])=>({name:l,value:f||0})),D=["#60A5FA","#A78BFA","#F472B6","#FB7185","#F59E0B","#34D399","#22D3EE","#93C5FD"],N="#CBD5E1",O=e.risks_to_analyze.R1?1e-5:e.risks_to_analyze.R3||e.risks_to_analyze.R4?.001:1e-5,k=_<=O?"#10B981":"#EF4444",U=E.map((l,f)=>{var g;return l.name==="R Total"?k:((g=y[f])==null?void 0:g.active)?D[f%D.length]:N}),j=["#F87171","#60A5FA","#A78BFA","#34D399","#F59E0B","#FB7185","#64748B"],Z=B("Componentes de Risco — Global",E,720,300,{barColors:U,labelAngle:-30,legendMaxWidth:420,toleranceLine:O}),b=[...v,{name:"F Total",value:((le=e.frequency_results)==null?void 0:le.F)||0}],C=(((ce=e.frequency_results)==null?void 0:ce.F)||0)<=L,z="#CBD5E1",J=b.map((l,f)=>{if(l.name==="F Total")return C?"#10B981":"#EF4444";const R=j[f%j.length];return(l.value||0)>0?R:z}),ne=B("Frequência de Danos — Global",b,720,280,{barColors:J,labelAngle:-40,legendFontSize:10,legendMaxWidth:500,toleranceLine:L});return`
...
`;continue}const L=r=>{let M=we(r);return M=M.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),M=M.replace(/(^|[^*])\*(?!\*)([^*]+?)\*(?!\*)/g,(B,w,y)=>`${w}<em>${y}</em>`),M};if(u.trim()==="---"){m&&(c+=`</ul>
`,m=!1),c+=`<hr style="border:0;border-top:1px solid #cbd5e1;margin:12px 0"/>
`;continue}const s=u.trim().match(/^!\[(.*?)\]\((.*?)\)$/);if(s){const[,r,M]=s;m&&(c+=`</ul>
`,m=!1);const B=n+1<F.length?F[n+1].trim():"",w=n-1>=0?F[n-1].trim():"",y=/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/;let T=null;const _=B.match(y),E=w.match(y);_?(T=`Figura ${_[2]} — ${_[3]}`.trim(),n+=1):E?T=`Figura ${E[2]} — ${E[3]}`.trim():r&&r.trim().length>0&&(T=r.trim());const v=T?`<figcaption>${L(T)}</figcaption>`:"";c+=`<figure style="margin:${t.figureMarginTopPx}px 0 ${t.figureMarginBottomPx}px"><img src="${we(M)}" alt="${we(r)}" style="max-width:100%;height:auto;border-radius:4px;border:none;display:inline-block;page-break-inside:avoid"/>${v}</figure>
...
`,m=!0);let r=u.trim().substring(2);for(;n+1<F.length&&F[n+1].startsWith("  ");)r+=" "+F[n+1].trim(),n++;const M=r.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),B=/\bresultado:\b/.test(M),w=/\bcalculo:\b/.test(M),y=/\bformula:\b/.test(M),T=/^\*\*[^*]+?\*\*:/.test(r.trim()),_=T?t.blockMarginBetweenItemsPx:Math.max(0,t.listItemMarginTopPx),E=T||B||w||y?t.blockMarginBetweenItemsPx:t.listItemMarginBottomPx,v=`${_}px 0 ${E}px`,D=/(F[óo]rmula:|C[áa]lculo:|Resultado:)/gi,N="§§",x=r.replace(D,N+"$1").split(N).filter(k=>k.trim().length>0);if(x.length>1||/:\s*$/.test(r.trim())){let k="";for(const U of x){const j=U.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),Z=/\bresultado:\b/.test(j),b=/\bcalculo:\b/.test(j),C=/\bformula:\b/.test(j),z=Z||b||C?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;k+=`<p style="margin:${z};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(U)}</p>`}c+=`<li style="margin:${v};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${k}</li>
`}else c+=`<li style="margin:${v};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(r)}</li>
`;continue}if(m&&(c+=`</ul>
`,m=!1),u.trim()){const r=n+1<F.length?F[n+1].trim():"";if(!(/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/.test(u.trim())&&/^!\[(.*?)\]\((.*?)\)$/.test(r))){const B=/(F[óo]rmula:|C[áa]lculo:|Resultado:)/gi,T=u.replace(B,"§§$1").split("§§").filter(_=>_.trim().length>0);if(T.length>1)for(const _ of T){const E=_.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),v=/\bresultado:\b/.test(E),D=/\bcalculo:\b/.test(E),N=/\bformula:\b/.test(E),O=v||D||N?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${O};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(_)}</p>
`}else{const _=u.replace(/\*/g,"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),E=/\bresultado:\b/.test(_),v=/\bcalculo:\b/.test(_),D=/\bformula:\b/.test(_),N=E||v||D?`${t.paragraphMarginTopPx}px 0 ${t.listItemExtraMarginBottomPx}px`:`${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px`;c+=`<p style="margin:${N};font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${L(u)}</p>
`}}}}return m&&(c+=`</ul>
`),c};function At({data:e,onUpdate:t}){const[a,F]=S.useState(!1),[c,m]=S.useState(""),[n,u]=S.useState(!1),[L,s]=S.useState(22),[r,M]=S.useState(15),[B,w]=S.useState(22),[y,T]=S.useState(6),[_,E]=S.useState(22),[v,D]=S.useState(6),N="report_format_prefs",O=()=>{try{const b=localStorage.getItem(N);if(b){const C=JSON.parse(b);return{...Ge,...C}}}catch{}return Ge},[x]=S.useState(O()),k=async()=>{F(!0),m("");try{const b=await bt(e);m(b)}finally{F(!1)}},U=async()=>{if(!c)return;const b=ke(c,x),C=c.replace(/\!\[[^\]]*\]\([^\)]*\)/g,"").replace(/^###\s+/gm,"").replace(/^##\s+/gm,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/^>\s+/gm,"").replace(/\n\n+/g,`

`);try{if(typeof window.ClipboardItem<"u"){const z=new window.ClipboardItem({"text/html":new Blob([b],{type:"text/html"}),"text/plain":new Blob([C],{type:"text/plain"})});await navigator.clipboard.write([z])}else await navigator.clipboard.writeText(C);u(!0),setTimeout(()=>u(!1),2e3)}catch{try{await navigator.clipboard.writeText(C)}catch{}u(!0),setTimeout(()=>u(!1),2e3)}},j=S.useMemo(()=>pt(e),[e.h,e.l,e.w,e.hp,e.ng,e.cd,e.has_electric_line,e.line_sections_1,e.use_adj_structure_1,e.l_adj_1,e.w_adj_1,e.h_adj_1,e.hp_adj_1,e.cd_adj_1,e.has_data_line,e.line_sections_2,e.use_adj_structure_2,e.l_adj_2,e.w_adj_2,e.h_adj_2,e.hp_adj_2,e.cd_adj_2]);S.useMemo(()=>Oe(e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),[e.probability_data,e.analyze_data_line_probabilities,e.has_data_line]);const Z=S.useMemo(()=>e.zones.map(b=>{const C=ut(b),z=Oe(b.probability_data||e.probability_data,e.analyze_data_line_probabilities,e.has_data_line),J=xt(z,b),ne=gt(j,J,C,e.selected_risk_components);return{zone:b,lossCalculations:C,riskCalculations:ne}}),[e.zones,j,e.selected_risk_components,e.analyze_data_line_probabilities,e.has_data_line,e.probability_data]);return S.useMemo(()=>ht(Z),[Z]),i.jsxs("div",{children:[i.jsxs(Ve,{children:[i.jsx(We,{className:"p-3",children:i.jsxs(Ue,{className:"flex items-center justify-between",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(He,{className:"w-5 h-5 text-slate-100"}),"Relatório Técnico Detalhado"]}),c&&!a&&i.jsx(Se,{variant:"outline",size:"icon",onClick:()=>m(""),className:"h-8 w-8 flex-shrink-0",children:i.jsx($t,{className:"w-4 h-4"})})]})}),c&&!a&&i.jsxs("div",{className:"flex justify-end gap-2 px-2 pb-0",children:[i.jsx(Se,{variant:"secondary",size:"sm",onClick:()=>{const b=ke(c,x),C=window.open("","_blank");if(!C)return;const z=Math.max(4,y),J=Math.max(4,v);C.document.write(`<!doctype html><html><head><meta charset="utf-8"/><title></title><style>
