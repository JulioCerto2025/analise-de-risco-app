...
<svg xmlns="http://www.w3.org/2000/svg" width="${F}" height="${h}" viewBox="0 0 ${F} ${h}">
  ...
  ${Ie}
  ${Te}
</svg>`;return`data:image/svg+xml;base64,${L(Je)}`},v=["RA","RB","RC","RM","RU","RV","RW","RZ"].map(l=>({name:l,value:e.risk_results[l]||1e-12,active:!!e.selected_risk_components[l]})),T=e.risks_to_analyze.R1?"R1":e.risks_to_analyze.R3?"R3":e.risks_to_analyze.R4?"R4":"R1",b=e.risk_results[T]||0,E=[...v.map(l=>({name:l.name,value:l.value})),{name:"R Total",value:b}],y=Object.entries(e.frequency_results||{}).filter(([l])=>l!=="F").map(([l,R])=>({name:l,value:R||0})),D=["#60A5FA","#A78BFA","#F472B6","#FB7185","#F59E0B","#34D399","#22D3EE","#93C5FD"],N="#CBD5E1",j=e.risks_to_analyze.R1?1e-5:e.risks_to_analyze.R3||e.risks_to_analyze.R4?.001:1e-5,w=b<=j?"#10B981":"#EF4444",H=E.map((l,R)=>{var h;return l.name==="R Total"?w:((h=v[R])==null?void 0:h.active)?D[R%D.length]:N}),k=["#F87171","#60A5FA","#A78BFA","#34D399","#F59E0B","#FB7185","#64748B"],V=B("Componentes de Risco — Global",E,720,300,{barColors:H,labelAngle:-30,legendMaxWidth:420,toleranceLine:j}),_=[...y,{name:"F Total",value:((le=e.frequency_results)==null?void 0:le.F)||0}],C=(((ce=e.frequency_results)==null?void 0:ce.F)||0)<=M,z="#CBD5E1",J=_.map((l,R)=>{if(l.name==="F Total")return C?"#10B981":"#EF4444";const F=k[R%k.length];return(l.value||0)>0?F:z}),ne=B("Frequência de Danos — Global",_,720,280,{barColors:J,labelAngle:-40,legendFontSize:10,legendMaxWidth:500,toleranceLine:M});return`
...
`;continue}if(/^\s*\*+\s*$/.test(p))continue;const M=r=>{let L=we(r);return L=L.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),L=L.replace(/(^|[^*])\*(?!\*)([^*]+?)\*(?!\*)/g,(B,I,v)=>`${I}<em>${v}</em>`),L=L.replace(/\*/g,""),L};if(p.trim()==="---"){m&&(c+=`</ul>
`,m=!1),c+=`<hr style="border:0;border-top:1px solid #cbd5e1;margin:12px 0"/>
`;continue}const s=p.trim().match(/^!\[(.*?)\]\((.*?)\)$/);if(s){const[,r,L]=s;m&&(c+=`</ul>
`,m=!1);const B=o+1<P.length?P[o+1].trim():"",I=o-1>=0?P[o-1].trim():"",v=/^(Figura|FIGURA)\s*([0-9]+(?:\.[0-9]+)*)[\.:\-]\s*(.*)$/;let T=null;const b=B.match(v),E=I.match(v);b?(T=`Figura ${b[2]} — ${b[3]}`.trim(),o+=1):E?T=`Figura ${E[2]} — ${E[3]}`.trim():r&&r.trim().length>0&&(T=r.trim());const y=T?`<figcaption>${M(T)}</figcaption>`:"";c+=`<figure style="margin:${t.figureMarginTopPx}px 0 ${t.figureMarginBottomPx}px"><img src="${we(L)}" alt="${we(r)}" style="max-width:100%;height:auto;border-radius:4px;border:none;display:inline-block;page-break-inside:avoid"/>${y}</figure>
`;continue}if(p.startsWith("> ")){m&&(c+=`</ul>
`,m=!1);const r=M(p.substring(2));c+=`<p style="margin:${t.paragraphMarginTopPx}px 0 ${t.paragraphMarginBottomPx}px;font-size:${t.paragraphFontSizePt}pt;line-height:${t.paragraphLineHeight}">${r}</p>
`;continue}if(p.startsWith("## ")){m&&(c+=`</ul>
`,m=!1),c+=`<h2 style="font-size:${t.h2FontSizeRem}rem;line-height:1.6;font-weight:700;margin:${t.h2MarginTopPx}px 0 ${t.h2MarginBottomPx}px;">${M(p.substring(3))}</h2>
`;continue}if(p.startsWith("### ")){m&&(c+=`</ul>
`,m=!1),c+=`<h3 style="font-size:${t.h3FontSizeRem}rem;line-height:1.5;font-weight:700;margin:${t.h3MarginTopPx}px 0 ${t.h3MarginBottomPx}px;">${M(p.substring(4))}</h3>
...
`)}}}}return m&&(c+=`</ul>
`),c};function At({data:e,onUpdate:t}){const[a,P]=S.useState(!1),[c,m]=S.useState(""),[o,p]=S.useState(!1),[M,s]=S.useState(22),[r,L]=S.useState(15),[B,I]=S.useState(22),[v,T]=S.useState(6),[b,E]=S.useState(22),[y,D]=S.useState(6),N="report_format_prefs",j=()=>{try{const _=localStorage.getItem(N);if(_){const C=JSON.parse(_);return{...Ge,...C}}}catch{}return Ge},[x]=S.useState(j()),w=async()=>{P(!0),m("");try{const _=await _t(e);m(_)}finally{P(!1)}},H=async()=>{if(!c)return;const _=ke(c,x),C=c.replace(/\!\[[^\]]*\]\([^\)]*\)/g,"").replace(/^###\s+/gm,"").replace(/^##\s+/gm,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/^>\s+/gm,"").replace(/\n\n+/g,`
